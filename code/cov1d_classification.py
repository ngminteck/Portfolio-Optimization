...
Model_Type = "conv1d_classification"
...
def conv1d_classification_hyperparameters_search(X, y, gpu_available, ticker_symbol):
    ...
    def conv1d_classification_objective(trial):
    ...
    study.optimize(conv1d_classification_objective, n_trials=100)
    ...
    for i in range(4, -1, -1):
        ...
        if key.startswith('old'):
            ...
            model = Conv1DModel(in_channels, trial_params['out_channels'], trial_params['kernel_size'],
                                     trial_params['num_blocks'], trial_params['l2_lambda'],
                                trial_params['dropout_rate'], classification=True).to(device)
...
def conv1d_classification_resume_training(X, y, gpu_available, ticker_symbol, hyperparameter_search=False, delete_old_data=False):
    ...
    if not all_existed or hyperparameter_search:
        conv1d_classification_hyperparameters_search(X, y, gpu_available, ticker_symbol)
