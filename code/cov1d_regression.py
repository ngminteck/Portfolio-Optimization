...
def conv1d_regression_hyperparameters_search(X, y, gpu_available, ticker_symbol, pca):
    ...
    def conv1d_regression_objective(trial):
...
def conv1d_regression_resume_training(X, y, gpu_available, ticker_symbol, pca=False, hyperparameter_search=False,
    ...
    if hyperparameter_search_needed:
        conv1d_regression_hyperparameters_search(X, y, gpu_available, ticker_symbol, pca)
...
def conv1d_regression_predict(X, gpu_available, ticker_symbol, pca=False, no=1):
    device = torch.device('cuda' if gpu_available and torch.cuda.is_available() else 'cpu')
    Root_Folder = Model_Scaler_Folder
    if pca:
        Root_Folder = Model_PCA_Folder
